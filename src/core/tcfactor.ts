...
class TCFactor extends EventEmitter {
  ...
  private env: "cluster" | "electron" | "browser";
  private app: string;
  private thread;
  public executor: any;
  private static ___apps = {};
  private taskCallback = {}; //任务回调
  ...
  private async initEvent(thread) {
    let _this = this;
  ...
  private execute(method, ...args): Promise<any> {
    if (this.isMaster) {
